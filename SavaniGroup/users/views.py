...
class UserLogin(APIView):
    def post(self, request):
        ...
        if data['country_code'] and re.match(r'^\+\d{1,3}$', data['country_code']):
            if data['mobile_no'].isnumeric() and data['mobile_no'] and data['mobile_no'] != '':
                get_user = db.community_members.find_one(
                    {"country_code": data['country_code'], "mobile_no": data['mobile_no']})
                if get_user is not None:
                    if get_user['mobile_verified']:
                        if get_user['is_approved']:
                            ...
                            update_user = db.community_members.find_one_and_update(
                                {"country_code": data['country_code'], "mobile_no": data['mobile_no']}, new_obj, return_document=ReturnDocument.AFTER)
                            mobile_no = update_user['country_code'] + \
                                update_user['mobile_no']
...
class AddandDeleteFamilyMembersAPI(APIView):
    def post(self, request):
        ...
        if token and ObjectId().is_valid(token["_id"]):
            data = request.data
            parent_user = db.community_members.find_one(
                {"_id": ObjectId(token["_id"]), "role": "parent_user"})
            if parent_user:
                    ...
                    if data['relation'] and data['relation'] != '' and data['relation'].isalpha():
                        if data["marital_status"] and (data["marital_status"] == 'married' or data["marital_status"] == 'unmarried' or data["marital_status"] == 'widow'):
                            if data["education"]:
                                if data["occupation"] and (data["occupation"] == 'business' or data['occupation'] == 'job' or data['occupation'] == 'other'):
                                    if data["aadhar_number"] and len(data["aadhar_number"]) == 12 and data['aadhar_number'].isnumeric():
                                        existingUser = db.community_members.find_one(
                                            {'aadhar_number': data["aadhar_number"]})
                                        if not existingUser:
                                            obj = {
    ...
    def delete(self, request):
        ...
        if token and ObjectId().is_valid(token["_id"]):
            ...
            parent_user = db.community_members.find_one(
                {"_id": ObjectId(token["_id"]), "role": "parent_user"})
            if parent_user:
                get_member = db.community_members.find_one(
                    {"_id": ObjectId(data["_id"]), "role": "child_user"})
                if get_member:
    ...
    def get(self, request):
        ...
        if token and ObjectId().is_valid(token["_id"]):
            ...
            if parent_user:
                ...
                family_members = valuesEntity(db.community_members.find(filter, {
                                              "password": 0, "createdBy": 0, "updatedBy": 0, "is_approved": 0, "createdAt": 0, "updatedAt": 0, "is_active": 0, "role": 0}))
                if family_members:
...
class UserProfileAPI(APIView):
    ...
    def get(self, request):
        token = authenticate(request)
        if token and ObjectId().is_valid(token["_id"]):
            userData = valueEntity(db.community_members.find_one({"_id": ObjectId(token["_id"]), "is_approved": True, "is_active": True, "registration_fee": True}, {
                                   "password": 0, "createdBy": 0, "updatedBy": 0, "is_approved": 0, "createdAt": 0, "updatedAt": 0}))
    ...
    def post(self, request):
        ...
        if token and ObjectId().is_valid(token["_id"]):
            data = request.data
            userData = db.community_members.find_one({"_id": ObjectId(
                token["_id"]), "is_approved": True, "is_active": True, "registration_fee": True})
            if userData is not None:
                new_obj = {"$set": {
                    "firstname": data["firstname"],
                    "lastname": data["lastname"],
                ...
                updateUser = db.community_members.find_one_and_update(
                    {"_id": ObjectId(token["_id"])}, new_obj)
                if updateUser:
                    updatedUser = valueEntity(db.community_members.find_one({"_id": ObjectId(token["_id"])}, {
                                              "password": 0, "createdBy": 0, "updatedBy": 0, "is_approved": 0, "createdAt": 0, "updatedAt": 0}))
    ...
    def delete(self, request):
        token = authenticate(request)
        if token and ObjectId().is_valid(token["_id"]):
            get_user = db.community_members.find_one({"_id": ObjectId(
                token["_id"]), "is_approved": True, "is_active": True, "registration_fee": True})
            if get_user:
            ...
            if parent_user:
                ...
                if data['family_member']:
                    family_member = db.community_members.find_one({"_id": ObjectId(
                        data['family_member']), "createdBy": parent_user['_id'], 'role': 'child_user'})
                    if family_member:
                        if data['medium']:
                                ...
                                if data['field'] or data['field'] == "":
                                    if data['percentage'] and (data['percentage'] >= 0 and data['percentage'] <= 100):
                                        if data['result'] or data['result'] == "":
                                            existingResult = db.student_results.find_one(
                                                {'parent_user': parent_user['_id'], 'family_member': family_member['_id']})
                                            if not existingResult:
                                                ...
                                                result = db.student_results.insert_one(
                                                    obj)
                                                if result:
...
class QualifiedStudentListAPI(APIView):
    def get(self, request):
        token = authenticate(request)
        if token and ObjectId().is_valid(token['_id']):
            user = db.community_members.find_one({"_id": ObjectId(token["_id"]), "registration_fees": True, "is_approved": True, "is_active": True, "role": "parent_user"}) or db.admin.find_one({
                "_id": ObjectId(token["_id"]), "is_active": True, "is_admin": True})
            if user is not None:
                standard = request.GET.get('standard')
                field = request.GET.get('field')
                if standard and standard != '':
                    results = valuesEntity(db.student_results.find({"standard": int(standard), "field": field, "status": "completed"}, {
                                           "createdAt": 0, "updatedAt": 0, "createdBy": 0, "updatedBy": 0}).sort("percentage", -1))
...
class WidowWomenHelpAPI(APIView):
    ...
    def post(self, request):
        ...
        if token and ObjectId().is_valid(token["_id"]):
            parent_user = db.community_members.find_one({"_id": ObjectId(
                token["_id"]), "registration_fees": True, "is_approved": True, "is_active": True, "role": "parent_user"})
            if parent_user:
                data = request.data
                if data["service"] in services:
                    if data['family_member']:
                        family_member = db.community_members.find_one({"_id": ObjectId(
                            data['family_member']), "createdBy": parent_user['_id'], 'role': 'child_user'})
                        if family_member:
                                                                        ...
                                                                        if data["running_service"] == True:
                                                                            if data["trust_name"]:
                                                                                if data["decided_amount_in_cash"] and data["decided_amount_in_school_fee"] and data["decided_total_amount"]:
                                                                                    # child_details = {"son":data['son'],
                                                                                    #                 "daughter" : data["daughter"]}
                                                                                    # decided_amount = {"cash": data["cash"],
                                                                                    ...
                                                                                    # obj["child_details"] = child_details
                                                                                    # obj["decided_amount"] = decided_amount
                                                                                    result = db.community_services.insert_one(
                                                                                        obj)
                                                                                    if result:
                                                                            ...
                                                                            obj = {
                                                                                    ...
                                                                                    # "decided_amount_in_cash": int(data["decided_amount_in_cash"]),
                                                                                    # "decided_amount_in_school_fee": int(data["decided_amount_in_school_fee"]),
                                                                                    # "decided_total_amount": int["decided_total_amount"],
                                                                                    "status": "inprocess",
                                                                                    "createdAt": datetime.datetime.now(),
                                                                                    "updatedAt": "",
                                                                                    "createdBy": ObjectId(parent_user["_id"]),
